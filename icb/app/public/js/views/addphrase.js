    ...
    $('#account-form').ajaxForm({
        error: function (e) {
            if (e.responseText == 'title-empty') {
                av.showInvalidTitle('Please Enter Title for the Phrase.');
            }
        },
        success: function (responseText, status, xhr, $form) {
            if (status == 'success')
                that.showPhraseAlert('Phrase has been created.</br>Click OK to return to the home page.');
        }
    });
...
// customize the account signup form //
    ...
    $('#account-form h2').text('Add Phrase');
    $('#account-form #sub').text('Please fill in the details of the phrase');
...
// setup the alert that displays when an phrase is successfully created //
    ...
    this.showPhraseAlert = function(msg){
        $('.modal-alert').modal({ show:false, keyboard : false, backdrop : 'static' });
        $('.modal-alert .modal-header h4').text('Phrase Created!');
