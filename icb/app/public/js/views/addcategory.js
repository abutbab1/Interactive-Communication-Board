    ...
    $('#account-form').ajaxForm({
        error : function(e){
            if (e.responseText == 'title-empty'){
                av.showInvalidTitle('Please Enter Title for the Category.');
            }
        },
        success	: function(responseText, status, xhr, $form){
            if (status == 'success')
                that.showCategoryAlert('Category has been created.</br>Click OK to return to the home page.');
        }
    });
...
// customize the account signup form //
    ...
    $('#account-form h2').text('Add Category');
    $('#account-form #sub').text('Please fill in the details of the category');
    $('#account-form-btn1').html('Cancel');
    $('#account-form-btn2').html('Create');
    $('#account-form-btn1').click(function(){ window.location.href = '/';});

// setup the alert that displays when a category is successfully created //

    this.showCategoryAlert = function(msg){
